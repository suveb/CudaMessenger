...
$messageList = "SELECT a.userid,CONCAT(firstname,' ',lastname) as name FROM users a WHERE userid IN ((SELECT m.userid FROM messages m WHERE m.userid IN ((SELECT f1.friendid as userid FROM friends f1 WHERE f1.userid = $userid)UNION(SELECT f2.userid FROM friends f2 WHERE f2.friendid = $userid)) AND m.friendid = $userid GROUP BY m.userid) UNION (SELECT m2.friendid FROM messages m2 WHERE m2.friendid IN ((SELECT f3.friendid as userid FROM friends f3 WHERE f3.userid = $userid) UNION (SELECT f4.userid FROM friends f4 WHERE f4.friendid = $userid)) AND m2.userid = $userid GROUP BY m2.friendid))";
...
$messageList = mysqli_query($conn,$messageList);
$messageList = mysqli_fetch_all($messageList,MYSQLI_ASSOC);
                        ...
                        <div class="card-block p-2">
                            <div class="post card-title">
                                <div class="post-title">
                                    <a href="messages.php?friendid=<?php echo $row['userid'] ?>"><?php echo $row['name'] ?></a>
                                </div>
                            </div>
                        </div>
                    </div>
