...
if(isset($_POST['submit'])){
    $comment = $_POST['comment'];
        $insertcomment = "INSERT INTO interacts(userid,postid,comments) VALUES($userid,$postid,'$comment') ON DUPLICATE KEY UPDATE comments='$comment'";
...
if(isset($_GET['liked'])){
    $changelike = "INSERT INTO interacts(userid,postid,liked) VALUES($userid,$postid,1) ON DUPLICATE KEY UPDATE liked = IF(liked,0,1)";
    if(mysqli_query($conn,$changelike)){
    }else{
        echo 'failed'.mysqli_error($conn);
    }
}
...
$posts = "SELECT p.name,o.datecreated,o.content,p.pageid FROM pages p,post o WHERE postid=$postid AND p.pageid = o.pageid";

$likes = "SELECT COUNT(liked) AS number FROM interacts WHERE postid = $postid AND liked = 1";

$comments = "SELECT comments,CONCAT(firstname,' ',lastname) as name FROM interacts i,users u WHERE postid = $postid AND i.userid = u.userid AND comments != ''";
...
$comments = mysqli_query($conn,$comments);
                    ...
                    <div class="card-block p-5">
                        <div class="post card-title">
                            <div class="post-title">
                                <a href="pages.php?pageid=<?php echo $posts[0]['pageid'] ?>"><?php echo $posts[0]['name']; ?></a>
                                <p class="text-muted"><?php echo $posts[0]['datecreated']; ?></p>
                            </div>
                        ...
                        <div class="interact mt-3">
                            <a class="d-block align-center py-1 float-left border border-blue rounded-lg" href="posts.php?postid=<?php echo $postid."&liked=1";?>">
                                <?php echo $likes[0]['number']." Likes";?>
                            </a>
                        ...
                        <div class="container">
                            <form action="posts.php?postid=<?php echo $postid ?>" class="needs-validation mx-auto" method="post" novalidate>
                                <div class="form-group">
                                    <label for="comment">Your Comment:</label>
                                    <input type="textarea" style="height:200px" class="form-control" name="comment" required autocomplete="off">
                                </div>
                        ...
                        <div class="jumbotron jumbotron-fluid rounded">
                            <div class="container">
                                <h5 class=""><?php echo $row['name']; ?></h1>
                                    <p class=" text-s pl-5 ml-5"><?php echo $row['comments']; ?></p>
                            </div>
                        </div>
                        <?php } ?>
                    </div>
