...
if(isset($_POST['search'])){
    $query = $_POST['query'];
    $pagelist = "SELECT pageid,name,description FROM pages WHERE name LIKE '%$query%'";
    $userlist = "SELECT userid,CONCAT(firstname,' ',lastname) AS name,emailid,description FROM users WHERE CONCAT(firstname,' ',lastname) LIKE '%$query%'";
    
    $pagelist = mysqli_query($conn,$pagelist);
    $pagelist = mysqli_fetch_all($pagelist,MYSQLI_ASSOC);
    
    $userlist = mysqli_query($conn,$userlist);
    $userlist = mysqli_fetch_all($userlist,MYSQLI_ASSOC);
}
            ...
            <div class="col-8 overflow-auto" style="height: 510px">
                    ...
                    <div class="card-block px-5 py-3">
                        <div class="post card-title">
                            <div class="post-title">
                                <a href="profile.php?userid=<?php echo $row['userid']; ?>"><?php echo $row['name']; ?></a>
                                <p class="text-muted"><?php echo $row['emailid']; ?></p>
                            </div>
                ...
                <div class="card list my-3 shadow">
                    <div class="card-block px-5 py-3">
                        <div class="post card-title">
                            <div class="post-title">
                                <a href="pages.php?pageid=<?php echo $row['pageid']; ?>"><?php echo $row['name']; ?></a>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                        <p class="lead"><?php echo $row['description']; ?></p>
